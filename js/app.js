...
function aiMove() {
  ...
  Bridge.search(game.moves.slice(), state.level, (info) => {
    ...
    let pick;
    let source = 'ai';
    if (result.forced !== 'none' || result.vcf || result.candidates.length <= 1) {
      pick = { move: result.move, score: result.candidates[0] ? result.candidates[0].score : 0, suboptimal: false };
      if (result.vcf) source = 'ai-vcf';
    } else {
      pick = Difficulty.pickMove(result.candidates, state.level);
